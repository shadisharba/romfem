...
for fixed_point = 1:5 %parameter
    old_temporal_mode = temporal_mode;
    
    b = full(numerical_model.submesh.integral_gradient_operator' * temporal_mass_integration(local_obj.minus_residual, temporal_mode, numerical_model.temporal));
    
    % KA0: zero correction for the dofs with imposed displacement
    displacement_spatial_mode(numerical_model.boundary_conditions.free_dof) = numerical_model.submesh.K_FF \ b(numerical_model.boundary_conditions.free_dof,1);
    
    strain_spatial_mode = numerical_model.submesh.gradient_operator * displacement_spatial_mode;

    % this normalisation ensures not having very small numbers in the
    % temporal_update and a faster convergence of the fixed point 
    % iteration [for fixed_point = 1:5]
    strain_spatial_mode = strain_spatial_mode / norm(strain_spatial_mode);
