function local_fields = local_stage_horizontal(numerical_model, old_global_fields, solver_parameters, iter)
...
if iter <= -3
    ...
    if ~solver_parameters.strain_pgd
        stress = full(old_global_fields.stress);
    else
    ...
    if nnz(row)
        ...
        damage_rate(:, col) = damage_evolution(material, Y, plastic_multiplier, old_damage, 1);
