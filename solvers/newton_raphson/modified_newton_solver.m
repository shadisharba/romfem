function [global_fields, relative_err] = modified_newton_solver(numerical_model_obj, global_fields)
...
for time_step = 2:length(numerical_model_obj.temporal.mesh)
    ...
    local_fields = update_internal_variables(numerical_model_obj, strain, global_fields.initial_values, time_step, false);
    ...
    while norm(relative_err) > numerical_model_obj.solver_parameters.convergence_tol && iter < numerical_model_obj.solver_parameters.max_iter
        ...
        delta_displacement = submesh.K_FF \ minus_residual;
