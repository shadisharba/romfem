function local_fields = local_stage_horizontal(numerical_model, old_global_fields, solver_parameters)
...
if max(equivalent_apparent_stress(:)) > 2 * material.user_material.sigma_y
    local_fields = local_stage_vertical(numerical_model, old_global_fields, solver_parameters);
    return
    %     stress = stress / 1.5;
end
...
elastic_strain = compute_elastic_strain(stress, material.inv_elasticity_tensor_diagonal, repelem(internal_damage, ncomp, 1));
elastic_strain(row6, :) = compute_elastic_strain(stress(row6, :), inv_elasticity, repelem(internal_damage(row, :), ncomp, 1));
