...
\frame{\frametitle{MOR for nonlinear problems}
Short-term plastic/damage computations
\begin{itemize}[leftmargin=1cm]
	\footnotesize
	\item Proper Orthogonal Decomposition (POD)\\
	\qquad [Kerfriden, 2012; Ryckelynck, 2011]. % Reese
%		\qquad [Kerfriden, 2011-2012; Ryckelynck, 2005-2011]. % Reese
\item \only<1>{Proper Generalised Decomposition (PGD)\\}
\only<2>{\colorbox{luhGreen}{Proper Generalised Decomposition (PGD)}\\}
	\qquad [Vitse, 2016; Bhattacharyya, 2017]. % Allix, 1989; 
\end{itemize}
%\vfill
Long-term fatigue computations
\begin{itemize}[leftmargin=1cm]
	\footnotesize
	\item Temporal homogenisation \\ {\qquad [Fish and Yu, 2002; Devulder, 2010]}
	\item	 \only<1>{(Modified) jump cycle approach \\}
	 \only<2>{\colorbox{luhGreen}{(Modified) jump cycle approach}\\}
	  {\qquad[Desmorat, 2005; Bhattacharyya, 2018]}
	\item 	\only<1>{Space-time finite element method \\}
	\only<2>{\colorbox{luhGreen}{Space-time finite element method}\\}	
	{\qquad[Bhamare, 2014; Fritzen, 2018]} % Oden 1969
\end{itemize}
\begin{textblock*}{\textwidth}(12mm,0.88\textheight)
	\begin{block}<2>{\centering PGD in a modified LATIN framework}
	\end{block}
\end{textblock*}
}
...
\twocol{
	{\hspace*{0.25cm} Static admissibility }
		\vspace{-0.3cm}
		\begin{align*}
		\div{\fsigma}+\fb & =\fzero && \text{ in } \mathrm{\Omega} \times {\color{red} [0,T]}  \\
		\fsigma \cdot \fn & =\bar{\boldsymbol{t}} && \text{ on } \mathrm{\partial\Omega_\mathrm{N}} \times {\color{red} \cI}
		\end{align*}}{
{\hspace*{0.25cm} Kinematic admissibility}
\vspace{-0.3cm}
\begin{align*}
\centering
\feps & = \grads{\fu} && \text{ in } \mathrm{\Omega} \times {\color{red} [0,T]}  \\
\fu & =\bar{\fu} && \text{ on } \mathrm{\partial\Omega_\mathrm{D}} \times {\color{red} \cI}
\end{align*}}{}
...
\frame{\frametitle{LATIN vs. incremental schemes}
	An \textbf{incremental} scheme:\\[0.2cm]
	Solve {\color{luhBlue} iteratively}\\[0.2cm]
	\begin{adjustbox}{max totalsize={.8\textwidth}{.7\textheight},center}
		\begin{tikzpicture}
			...
			\draw [ultra thick,<->, >=stealth] (0.7,-2) -- node [above]{(\textbf{AD.})} (4cm,-2cm) ;
		\end{tikzpicture}
	\end{adjustbox}\\
...
\frame{\frametitle{LATIN vs. incremental schemes}
		...
		Solve {\color{luhBlue}iteratively} {\color{red} (\textbf{AD.})} and {\color{luhBlue}(\textbf{CM.})} \\[0.2cm]
		\begin{adjustbox}{max totalsize={.8\textwidth}{.7\textheight},center}
			\begin{tikzpicture}
			\node [block] (primal) {Primal variable $\fu$};
			\node [cloud, left=0.5cm of primal] (dirichlet) {Dirichlet B.C.};
...
\only<2>{\item\colorbox{luhGreen}{Solve {\color{red} (\textbf{AD}.)} to get {\color{red} ${s}_{\rm global}$} (global stage)} \\[0.3cm]}
...
\frame{\frametitle{Static admissibility}
	\begin{itemize}
		...
		\begin{equation*}
		\intspt \grads{\fw^\ast} : {\color{red} {\fsigma}^{(i+1)}_{\rm global}} \dspt = \intspt \fw^\ast \cdot \fb \ \d \Omega \d t+ \int \displaylimits_{\pOmegaN\times\cI}   \fw^\ast \cdot \bar{\ft} \ \d S \d t
		\end{equation*}
%		\vfill
		\item Correction $\Delta{\bullet}^{(i+1)}_{\rm global}={\bullet}^{(i+1)}-{\bullet}^{(i)}$
		\begin{align*}
		\intspt \grads{\fw^\ast} : {\color{red} \Delta {\fsigma}^{(i+1)}_{\rm global}} \ \d \Omega \d t = 0
		\end{align*}
%		\vfill
		\item Static admissibility and global search direction
		\begin{align*}
		\intspt  \grads{\fw^\ast} : {\ffH^-} : {\color{red} \Delta {\feps}^{(i+1)}_{\rm global}} \ \d \Omega \d t & = - \intspt \grads{\fw^\ast} : \underbrace{\color{luhBlue} \hf_{\text{local}}}_{\color{luhBlue} \text{known}}  \ \d \Omega \d t
		\end{align*}
	\end{itemize}
}
...
\framesubtitle{Galerkin finite element discretisation}
	\begin{itemize}
		...
		\item Spatial problem, with homogeneous boundary conditions: 
			...
			\end{align*}
\vfill
		\item Temporal problem, with zero initial conditions:
		\begin{align*}
		\footnotesize
...
\frame{\frametitle{What is actually solved?}
\begin{itemize}
\item	Choosing the search directions ${\ffH^-}$ as
\begin{equation*}
\footnotesize
{\ffH^-} = \alpha \ \ffC \qquad \alpha \in \ ]0,1]
\end{equation*}
\vfill
\item Enrichment step
	\begin{align*}
	\gamma\ \mK\ \vec{v} &= \vec{f} \qquad\ \gamma \in \ffR \qquad \mK \in \ffR^{\color{red} n \times n} \qquad \vec{v}, \vec{f} \in \ffR^{\color{red} n} &&\text{ with B.C.}\\
	a\ \vlambda &= \vb  \qquad\ \ \, a \in \ffR \qquad \ \, \vlambda, \vb \in \ffR^{\color{blue} n_t}  &&\text{ with I.C.}
	\end{align*}
\vfill
\item Temporal update step
\begin{equation*}
\begin{split}
\mat{\z{A}} \ \T{\z{\mat{\Lambda}}} = \mat{\z{B}}  \qquad \ \z\mA \in \ffR^{\color{blue} \mu \times \mu} \qquad \mat{\z{B}} \in \ffR^{\color{blue} \mu  \times n_t} \qquad \z{\mat{\Lambda}}=[\Delta\vlambda_{1},\cdots,\Delta\vlambda_{\mu}]
\end{split}
\end{equation*}
\end{itemize}
}
...
\frame{\frametitle{Challenges in a PGD framework} % Goals // challenges 
	...
	\begin{itemize} % \frownie{} \dSmiley
		\item The cost of integration over all generalised coordinates
		\begin{align*}
		\intT \intS \bullet \ \d \Omega \d t \qquad {\color{gray}\frownie}
		\end{align*}
		\vfill
		\item The fast increase in the number of modes
		\begin{align*}
		\fu(\fx,t) = \sum_{j=1}^{\mu} {\fv_j}(\fx) \circ \flambda_j(t) \text{ with large } \mu \quad {\color{red} \frownie}
		\end{align*}
		\vfill
		\item Efficient variable amplitude and frequency simulations\\[0.5cm]
%		\item Sometimes convergence is an issue 
	\end{itemize}
}
...
\frame{\frametitle{SVD Compression of PGD}
	\begin{itemize}
		...
		\end{align*}
		\vfill
		\item PGD enrichment with a Gram-Schmidt scheme is not optimal
...
\frame[t]{\frametitle{Is this efficient?}
	\begin{itemize}
		\item A low-rank approximation of ${\color{red} {\feps}_{\rm global}}$
		\item What about $\color{red} {\fsigma}_{\rm global}$, the local stage and the error indicator?
		\begin{align*}
		( {\color{red} {\fsigma}_{\rm global}} - {\color{luhBlue} {\fsigma}_{\rm local}} ) -{\ffH^\mp} : ( {\color{red} {\feps}_{\rm global}} - {\color{luhBlue} {\feps}_{\rm local}} ) = \fzero
		\end{align*}
		\vfill
%		\pause
		\item What is missing is PGD of ${\color{luhBlue} {\feps}_{\rm local}}$			
%		\pause
		\vfill
		\item Randomised SVD (RSVD) algorithm
		\item A dedicated arithmetic toolbox to handle operations on PGD
	\end{itemize}
}
